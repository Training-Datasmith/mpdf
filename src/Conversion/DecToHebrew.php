...
class Dec_To_Hebrew
{
    public function convert($in, $reverse = false)
    {
        ...
        $additive_nums_count = count($additive_nums);
        for ($t = 0; $t < $additive_nums_count; $t++) {
            ...
            for ($j = 0; $j < $n; $j++) {
                if (is_array($additive_glyphs[$t])) {
                    foreach ($additive_glyphs[$t] as $ag) {
                        if ($reverse) {
                            $s = Utf_String::code2utf($ag) . $s;
                        } else {
                            $s .= Utf_String::code2utf($ag);
                        }
                    }
                } else if ($reverse) {
                    $s = Utf_String::code2utf($additive_glyphs[$t]) . $s;
                } else {
                    $s .= Utf_String::code2utf($additive_glyphs[$t]);
                }
                $i -= $ct * $n;
            }
            if ($i === 0.0 || $i === 0) {
