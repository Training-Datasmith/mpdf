 ...
 */
class Postnet extends \Mpdf\Barcode\Abstract_Barcode implements \Mpdf\Barcode\Barcode_Interface
{
    /**
     * @param string $code
     * @param float $xDim
     ...
     * @param bool $planet
     */
    public function __construct($code, $x_dim, $gap_width, $planet = false)
    {
        $this->init($code, $gap_width, $planet);
        $this->data['nom-X'] = $x_dim;
        $this->data['nom-H'] = 3.175;
     ...
     * @param bool $planet
     */
    private function init($code, $gap_width, $planet)
    {
        // bar lenght
        if ($planet) {
        ...
        $bararray['bcode'][$k++] = ['t' => 0, 'w' => $gap_width, 'h' => 5, 'p' => 0];
        ...
        for ($i = 0; $i < $len; ++$i) {
            for ($j = 0; $j < 5; ++$j) {
                ...
                $bararray['bcode'][$k++] = ['t' => 0, 'w' => $gap_width, 'h' => 2, 'p' => 0];
                $bararray['maxw'] += 1 + $gap_width;
            }
        }
        // end bar
