 ...
 */
class Code11 extends \Mpdf\Barcode\Abstract_Barcode implements \Mpdf\Barcode\Barcode_Interface
{
    /**
     * @param string $code
    ...
    private function init($code, $print_ratio)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            if (!isset($chr[$code[$i]])) {
                throw new \Mpdf\Barcode\Barcode_Exception(sprintf('Invalid character "%s" in CODE11 barcode value "%s"', $code[$i], $code));
            }
            $seq = $chr[$code[$i]];
            for ($j = 0; $j < 6; ++$j) {
                $t = $j % 2 === 0;
