 ...
 */
class Codabar extends \Mpdf\Barcode\Abstract_Barcode implements \Mpdf\Barcode\Barcode_Interface
{
    /**
     * @param string $code
     * @param float $printRatio
     */
    public function __construct($code, $print_ratio, $quiet_zone_left = null, $quiet_zone_right = null)
    {
        $this->init($code, $print_ratio);
     ...
     * @param float $printRatio
     */
    private function init($code, $print_ratio)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            if (!isset($chr[$code[$i]])) {
                throw new \Mpdf\Barcode\Barcode_Exception(sprintf('Invalid character "%s" in CODABAR barcode value "%s"', $code[$i], $code));
            }
            $seq = $chr[$code[$i]];
            for ($j = 0; $j < 8; ++$j) {
                if ($j % 2 == 0) {
