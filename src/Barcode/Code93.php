 ...
 */
class Code93 extends \Mpdf\Barcode\Abstract_Barcode implements \Mpdf\Barcode\Barcode_Interface
{
    /**
     * @param string $code
     */
    ...
    private function init($code)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            if (ord($code[$i]) > 127) {
                throw new \Mpdf\Barcode\Barcode_Exception(sprintf('Invalid character "%s" in CODE93 barcode value "%s"', $code[$i], $code));
            }
        ...
        for ($i = 0; $i < $clen; ++$i) {
            ...
            if (!isset($chr[$char])) {
                // invalid character
                throw new \Mpdf\Barcode\Barcode_Exception(sprintf('Invalid CODE93 barcode value "%s"', $code));
            }
            for ($j = 0; $j < 6; ++$j) {
                if ($j % 2 == 0) {
                    $t = true;
