...
use Mpdf\Mpdf;
class Meter extends Inline_Tag
{
    public function open($attr, &$ahtml, &$ihtml)
    {
        $tag = $this->get_tag_name();
        $this->mpdf->in_meter = true;
        $max = 1;
        if (!empty($attr['MAX'])) {
            $max = $attr['MAX'];
        }
        ...
        $svg = $this->make_svg($type, $value, $max, $min, $optimum, $low, $high);
        //Save to local file
        ...
        if ($w == 0 && $h == 0) {
            ...
            $w = $this->mpdf->font_size / (10 / Mpdf::SCALE) * abs($info['w']) / Mpdf::SCALE;
            $h = $this->mpdf->font_size / (10 / Mpdf::SCALE) * abs($info['h']) / Mpdf::SCALE;
        }
    ...
    public function close(&$ahtml, &$ihtml)
    {
        ...
        $this->mpdf->in_meter = false;
    }
    protected function make_svg($type, $value, $max, $min, $optimum, $low, $high)
    {
        if ($type == '2') {
            /////////////////////////////////////////////////////////////////////////////////////
            ///////// CUSTOM <meter type="2">
