...
class Img extends Tag
{
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        if (isset($attr['SRC'])) {
            ...
            $properties = $this->css_manager->merge_css('', 'IMG', $attr);
            ...
            if (isset($properties['WIDTH'])) {
                ...
                $w = $this->size_converter->convert($attr['WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            }
            if (isset($properties['HEIGHT'])) {
                ...
                $h = $this->size_converter->convert($attr['HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            }
            $maxw = $maxh = $minw = $minh = false;
            if (isset($properties['MAX-WIDTH'])) {
                $maxw = $this->size_converter->convert($properties['MAX-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            } elseif (isset($attr['MAX-WIDTH'])) {
                $maxw = $this->size_converter->convert($attr['MAX-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            }
            if (isset($properties['MAX-HEIGHT'])) {
                $maxh = $this->size_converter->convert($properties['MAX-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            } elseif (isset($attr['MAX-HEIGHT'])) {
                $maxh = $this->size_converter->convert($attr['MAX-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            }
            if (isset($properties['MIN-WIDTH'])) {
                $minw = $this->size_converter->convert($properties['MIN-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            } elseif (isset($attr['MIN-WIDTH'])) {
                $minw = $this->size_converter->convert($attr['MIN-WIDTH'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            }
            if (isset($properties['MIN-HEIGHT'])) {
                $minh = $this->size_converter->convert($properties['MIN-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            } elseif (isset($attr['MIN-HEIGHT'])) {
                $minh = $this->size_converter->convert($attr['MIN-HEIGHT'], $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'], $this->mpdf->font_size, false);
            }
            ...
            if (!empty($properties['IMAGE-RENDERING'])) {
                ...
                if (strtolower($properties['IMAGE-RENDERING']) === 'crisp-edges') {
                    ...
                    $interpolation = $this->mpdf->interpolate_images;
                }
                $info['interpolation'] = $interpolation;
            }
            // Image file
            $info = $this->image_processor->get_image($srcpath, true, true, $orig_srcpath, $interpolation);
            // mPDF 6
            if (!$info) {
            ...
            $max_height = $this->mpdf->h - ($this->mpdf->t_margin + $this->mpdf->b_margin + 1);
            ...
            /* -- CSS-IMAGE-FLOAT -- */
            if (!$this->mpdf->col_active && !$this->mpdf->table_level && !$this->mpdf->listlvl && !$this->mpdf->kwt) {
            ...
            /* -- END CSS-IMAGE-FLOAT -- */
            // mPDF 5.7.3 TRANSFORMS
            if (isset($properties['TRANSFORM']) && !$this->mpdf->col_active && !$this->mpdf->kwt) {
