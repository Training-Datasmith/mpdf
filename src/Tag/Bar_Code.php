...
class Bar_Code extends Tag
{
    ...
    public function open($attr, &$ahtml, &$ihtml)
    {
        ...
        if (!empty($attr['CODE'])) {
            ...
            $properties = $this->css_manager->merge_css('', 'BARCODE', $attr);
            ...
            if (in_array($objattr['btype'], ['EAN13', 'ISBN', 'ISSN', 'UPCA', 'UPCE', 'EAN8'])) {
                ...
                $arrcode = $this->barcode->get_barcode_array($code, $objattr['btype'], '', $objattr['quiet_l'], $objattr['quiet_r']);
                if ($objattr['bsupp'] == 2 || $objattr['bsupp'] == 5) {
                    // EAN-2 or -5 Supplement
                    $supparrcode = $this->barcode->get_barcode_array($objattr['bsupp_code'], 'EAN' . $objattr['bsupp'], '', $objattr['quiet_l'], $objattr['quiet_r']);
                ...
                $arrcode = $this->barcode->get_barcode_array($objattr['code'], $objattr['btype'], '', $objattr['quiet_l'], $objattr['quiet_r']);
                ...
                $arrcode = $this->barcode->get_barcode_array($objattr['code'], $objattr['btype'], $objattr['pr_ratio'], $objattr['quiet_zone_left'], $objattr['quiet_zone_right']);
                $w = ($arrcode['maxw'] + $arrcode['lightmL'] + $arrcode['lightmR']) * $arrcode['nom-X'] * $objattr['bsize'];
