...
class Svg
{
     ...
     */
    public $size_converter;
    /**
     * @var \Mpdf\Color\ColorConverter
     */
    public $color_converter;
    /**
     * @var \Mpdf\Language\LanguageToFontInterface
     */
    public $language_to_font;
    /**
    ...
    public $ybase;
    public $svg_error;
    public $sub_path_init;
    public $spxstart;
    public $spystart;
    public $kp;
    // convert pixels to PDF units
    public $path_b_box;
    public $textlength;
    // mPDF 5.7.4
    public $texttotallength;
    // mPDF 5.7.4
    public $textoutput;
    // mPDF 5.7.4
    public $textanchor;
    ...
    private $in_defs;
    public function __construct(Mpdf $mpdf, Otl $otl, Css_Manager $css_manager, Image_Processor $image_processor, Size_Converter $size_converter, Color_Converter $color_converter, Language_To_Font_Interface $language_to_font, Script_To_Language_Interface $script_to_language)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
        ...
        $this->kf = 1;
        // constant To convert font size if re-mapped
        $this->path_b_box = [];
        $this->svg_style = [[
            'fill' => 'black',
            'fill-opacity' => 1,
            //	remplissage opaque par defaut
            'fill-rule' => 'nonzero',
            //	mode de remplissage par defaut
            'stroke' => 'none',
            //	pas de trait par defaut
            'stroke-linecap' => 'butt',
            //	style de langle par defaut
            'stroke-linejoin' => 'miter',
            'stroke-miterlimit' => 4,
            //	limite de langle par defaut
            'stroke-opacity' => 1,
            //	trait opaque par defaut
            'stroke-width' => 1,
            'stroke-dasharray' => 0,
            'stroke-dashoffset' => 0,
            'color' => '',
        ...
        $this->txt_style = [[
            'fill' => 'black',
            //	pas de remplissage par defaut
            'font-family' => $mpdf->default_font,
            'font-size' => $mpdf->default_font_size,
            // 	****** this is pts
            'font-weight' => 'normal',
            //	normal | bold
            'font-style' => 'normal',
    ...
    public function svg_image(array $attribs)
    {
        ...
        $w = $this->size_converter->convert($wset, $this->svg_info['w'] * (25.4 / $this->mpdf->dpi), $this->mpdf->font_size, false);
        $h = $this->size_converter->convert($hset, $this->svg_info['h'] * (25.4 / $this->mpdf->dpi), $this->mpdf->font_size, false);
        if ($w == 0 || $h == 0) {
            return;
        }
        ...
        if (trim($srcpath) != '' && substr($srcpath, 0, 4) == 'var:') {
            ...
            $this->mpdf->get_full_path($srcpath);
        }
        // Image file (does not allow vector images i.e. WMF/SVG)
        // mPDF 6 Added $this->mpdf->interpolateImages
        $info = $this->image_processor->get_image($srcpath, true, false, $orig_srcpath, $this->mpdf->interpolate_images);
        if (!$info) {
            return;
        }
        ...
        if ($meet_or_slice == 'slice') {
            ...
            $s = ' q 0 w ';
            // Line width=0
            $s .= sprintf('%.3F %.3F m ', $x * $this->kp, -($y + $h) * $this->kp);
            // start point TL before the arc
            $s .= sprintf('%.3F %.3F l ', $x * $this->kp, -$y * $this->kp);
            // line to BL
            $s .= sprintf('%.3F %.3F l ', ($x + $w) * $this->kp, -$y * $this->kp);
            // line to BR
            $s .= sprintf('%.3F %.3F l ', ($x + $w) * $this->kp, -($y + $h) * $this->kp);
            // line to TR
            $s .= sprintf('%.3F %.3F l ', $x * $this->kp, -($y + $h) * $this->kp);
            // line to TL
            $s .= ' W n ';
            // Ends path no-op & Sets the clipping path
            $this->svg_write_string($s);
        }
        $outstring = sprintf(' q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q ', $img_w * $this->kp, $img_h * $this->kp, $img_x * $this->kp, -($img_y + $img_h) * $this->kp, $info['i']);
        $this->svg_write_string($outstring);
        if ($meet_or_slice == 'slice') {
            // need to end clipping path
            $this->svg_write_string(' Q ');
        }
    }
    public function svg_gradient(array $gradient_info, array $attribs, $element)
    {
        ...
        if ($element == 'rect') {
            ...
            if (is_array($this->path_b_box) && $this->path_b_box[2] > 0) {
                $w = $this->path_b_box[2];
                $h = $this->path_b_box[3];
                $x_offset = $this->path_b_box[0];
                $y_offset = $this->path_b_box[1];
            } else {
        ...
        if ($gradient_info['type'] == 'linear') {
            ...
            if ($spread == 'R' || $spread == 'F') {
                ...
                while ($inside) {
                    ...
                    for ($i = 0; $i < $ns; $i++) {
                        ...
                        $px2 = $px1 + cos($alpha);
                        $py2 = $py1 + sin($alpha);
                        $res1 = $this->test_intersect($px1, $py1, $px2, $py2, 0, 0, 0, 1);
                        // $x=0 vert axis
                        $res2 = $this->test_intersect($px1, $py1, $px2, $py2, 1, 0, 1, 1);
                        // $x=1 vert axis
                        $res3 = $this->test_intersect($px1, $py1, $px2, $py2, 0, 0, 1, 0);
                        // $y=0 horiz axis
                        $res4 = $this->test_intersect($px1, $py1, $px2, $py2, 0, 1, 1, 1);
                        // $y=1 horiz axis
            ...
            for ($i = 0; $i < $stops; $i++) {
                ...
                $this->mpdf->gradients[$n]['stops'][] = ['col' => $gradient_info['color'][$i]['color'], 'opacity' => $gradient_info['color'][$i]['opacity'], 'offset' => $offset];
            ...
            if (isset($gradient_info['units']) && strtolower($gradient_info['units']) == 'userspaceonuse') {
                ...
                $angle = rad2deg(atan2($gradient_info['info']['y0'] - $ay1, $gradient_info['info']['x0'] - $ax1));
                if ($angle < 0) {
                    $angle += 360;
                } elseif ($angle > 360) {
                    $angle -= 360;
            ...
            if ($spread == 'R' || $spread == 'F') {
                ...
                while ($inside) {
                    ...
                    for ($i = 0; $i < $ns; $i++) {
                        ...
                        $py = $y1 + ($y0 - $y1) * $tmp;
                        $pr = $r * $tmp;
                        $res = $this->test_intersect_circle($px, $py, $pr);
                        if (!$res) {
                            $inside = false;
                        }
                    }
    ...
    public function svg_offset(array $attribs)
    {
        ...
        if (isset($attribs['width']) && $attribs['width']) {
            $svg_w = $this->size_converter->convert($attribs['width']);
            // mm (interprets numbers as pixels)
        }
        if (isset($attribs['height']) && $attribs['height']) {
            $svg_h = $this->size_converter->convert($attribs['height']);
            // mm
        }
        // mPDF 5.0.005
        ...
        $this->svg_info['x'] = 0;
        $this->svg_info['y'] = 0;
        $this->svg_info['w'] = $svg_w / 0.2645;
        // mm->pixels
        $this->svg_info['h'] = $svg_h / 0.2645;
        // mm->pixels
    }
    //
    ...
    public function svg_define_style(array $critere_style)
    {
        ...
        if (isset($critere_style['style'])) {
            ...
            // mPDF 5.7.2
            if ((preg_match("/[^-]opacity:\\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m) || preg_match("/^opacity:\\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m)) && $m[1] != 'inherit') {
    ...
    public function svg_style(array $critere_style, $attribs, $element)
    {
        $path_style = '';
        $fill_gradient = '';
        $w = '';
        ...
        if (substr_count($critere_style['fill'], 'url') > 0 && $element != 'line') {
            ...
            if ($id_gradient != $critere_style['fill']) {
                if (isset($this->svg_gradient[$id_gradient])) {
                    $fill_gradient = $this->svg_gradient($this->svg_gradient[$id_gradient], $attribs, $element);
                    if ($fill_gradient) {
                        $path_style = 'q ';
                        $w = 'W';
                        $style .= 'N';
            ...
            $col = $this->color_converter->convert($critere_style['color'], $this->mpdf->pdfa_xwarnings);
            if ($col) {
                ...
                // CMYKa
                $path_style .= $this->mpdf->set_f_color($col, true) . ' ';
                $style .= 'F';
            }
        } elseif ($critere_style['fill'] != 'none' && $element != 'line') {
            $col = $this->color_converter->convert($critere_style['fill'], $this->mpdf->pdfa_xwarnings);
            if ($col) {
                if ($col[0] == 5 && is_numeric($col[4])) {
                    $critere_style['fill-opacity'] = ord($col[4] / 100);
                }
        ...
        if (substr_count($critere_style['stroke'], 'url') > 0) {
            ...
            */
            ...
            if ($col) {
                ...
                // CMYKa
                $path_style .= $this->mpdf->set_d_color($col, true) . ' ';
                $style .= 'D';
                $lw = $this->convert_svg_size_pixels($critere_style['stroke-width']);
                $path_style .= sprintf('%.3F w ', $lw * $this->kp);
            }
        } elseif ($critere_style['stroke'] != 'none') {
            $col = $this->color_converter->convert($critere_style['stroke'], $this->mpdf->pdfa_xwarnings);
            if ($col) {
                // mPDF 5.0.051
                // mPDF 5.3.74
                if ($col[0] == 5 && is_numeric($col[4])) {
        ...
        if ($critere_style['stroke'] != 'none') {
            ...
            if (isset($critere_style['stroke-miterlimit'])) {
                if ($critere_style['stroke-miterlimit'] != 'none' && preg_match('/^[\d.]+$/', $critere_style['stroke-miterlimit'])) {
        ...
        if (isset($critere_style['fill-opacity'])) {
            ...
            $gs = $this->mpdf->add_ext_g_state(['ca' => $opacity, 'BM' => '/Normal']);
    ...
    public function svg_path($command, $arguments)
    {
        $path_cmd = '';
        $newsubpath = false;
        // mPDF 5.0.039
        $minl = $this->path_b_box[0];
        $mint = $this->path_b_box[1];
        $maxr = $this->path_b_box[2] + $this->path_b_box[0];
        $maxb = $this->path_b_box[3] + $this->path_b_box[1];
        ...
        switch (strtolower($command)) {
            ...
            case 'v':
                ...
                break;
            case 's':
                // bezier with first vertex equal first control
                // mPDF 4.4.003
                ...
                for ($i = 0; $i < $argument_count; $i += 4) {
                    ...
                    $this->lastcontrolpoints = [$pdfx - $pdfx2, -($pdfy - $pdfy2)];
                    // mPDF 4.4.003 always relative
                    $pdf_pt = $this->svg_overflow($pdfx, $pdfy);
                    $curves = [$pdfx1, -$pdfy1, $pdfx2, -$pdfy2, $pdfx, -$pdfy];
                    $bx = $this->compute_bezier_bounding_box($start, $curves);
                    $minl = min($minl, $bx[0]);
                    $maxr = max($maxr, $bx[2]);
                    $mint = min($mint, $bx[1]);
                    $maxb = max($maxb, $bx[3]);
                ...
                break;
            case 'c':
                // bezier with second vertex equal second control
                for ($i = 0; $i < $argument_count; $i += 6) {
                    $x1 = $a[$i];
                    $y1 = $a[$i + 1];
                    $x2 = $a[$i + 2];
                    $y2 = $a[$i + 3];
                ...
                break;
            case 'q':
                // bezier quadratic avec point de control
                for ($i = 0; $i < $argument_count; $i += 4) {
                    $x1 = $a[$i];
                    $y1 = $a[$i + 1];
                    $x = $a[$i + 2];
                ...
                break;
            case 't':
                // bezier quadratic avec point de control simetrique a lancien point de control
            ...
            case 'a':
                // Elliptical arc
                for ($i = 0; $i < $argument_count; $i += 7) {
                    $rx = isset($a[$i]) ? $a[$i] : 0;
                    $ry = isset($a[$i + 1]) ? $a[$i + 1] : 0;
                    // x-axis-rotation
                    $angle = isset($a[$i + 2]) ? $a[$i + 2] : 0;
                    $large_arc_flag = isset($a[$i + 3]) ? $a[$i + 3] : 0;
                    $sweep_flag = isset($a[$i + 4]) ? $a[$i + 4] : 0;
                    ...
                    list($pcmd, $bounds) = $this->Arcto($x1, $y1, $x2, $y2, $rx, $ry, $angle, $large_arc_flag, $sweep_flag);
        ...
        // mPDF 5.0.039
        $this->path_b_box[0] = $minl;
        $this->path_b_box[1] = $mint;
        $this->path_b_box[2] = $maxr - $this->path_b_box[0];
        $this->path_b_box[3] = $maxb - $this->path_b_box[1];
        return $path_cmd;
    }
    public function Arcto($x1, $y1, $x2, $y2, $rx, $ry, $angle, $large_arc_flag, $sweep_flag)
    {
        ...
        $sin_phi = sin(deg2rad($angle));
        $cos_phi = cos(deg2rad($angle));
        $x1dash = $cos_phi * ($x1 - $x2) / 2.0 + $sin_phi * ($y1 - $y2) / 2.0;
        $y1dash = -$sin_phi * ($x1 - $x2) / 2.0 + $cos_phi * ($y1 - $y2) / 2.0;
        ...
        if ($numerator < 0.0) {
            ...
            $root = ($large_arc_flag == $sweep_flag ? -1.0 : 1.0) * sqrt($numerator / ($rx * $rx * $y1dash * $y1dash + $ry * $ry * $x1dash * $x1dash));
        }
        $cxdash = $root * $rx * $y1dash / $ry;
        $cydash = -$root * $ry * $x1dash / $rx;
        $cx = $cos_phi * $cxdash - $sin_phi * $cydash + ($x1 + $x2) / 2.0;
        $cy = $sin_phi * $cxdash + $cos_phi * $cydash + ($y1 + $y2) / 2.0;
        $theta1 = $this->calc_vector_angle(1.0, 0.0, ($x1dash - $cxdash) / $rx, ($y1dash - $cydash) / $ry);
        $dtheta = $this->calc_vector_angle(($x1dash - $cxdash) / $rx, ($y1dash - $cydash) / $ry, (-$x1dash - $cxdash) / $rx, (-$y1dash - $cydash) / $ry);
        if (!$sweep_flag && $dtheta > 0) {
            $dtheta -= 2.0 * M_PI;
        } elseif ($sweep_flag && $dtheta < 0) {
            $dtheta += 2.0 * M_PI;
        }
        ...
        for ($i = 0; $i < $segments; $i++) {
            ...
            $xe = $cos_phi * $rx * $cos_theta2 - $sin_phi * $ry * $sin_theta2 + $cx;
            $ye = $sin_phi * $rx * $cos_theta2 + $cos_phi * $ry * $sin_theta2 + $cy;
            // b) calculate gradients at start/end points of segment:
            $dx1 = $t * (-$cos_phi * $rx * $sin_theta1 - $sin_phi * $ry * $cos_theta1);
            $dy1 = $t * (-$sin_phi * $rx * $sin_theta1 + $cos_phi * $ry * $cos_theta1);
            $dxe = $t * ($cos_phi * $rx * $sin_theta2 + $sin_phi * $ry * $cos_theta2);
            $dye = $t * ($sin_phi * $rx * $sin_theta2 - $cos_phi * $ry * $cos_theta2);
    ...
    public function calc_vector_angle($ux, $uy, $vx, $vy)
    {
        $ta = atan2($uy, $ux);
        $tb = atan2($vy, $vx);
        if ($tb >= $ta) {
    ...
    public function convert_svg_size_pixels($size = 5, $maxsize = 'x')
    {
        ...
        $maxsize *= 25.4 / $this->mpdf->dpi;
        // convert pixels to mm
        $fontsize = $this->mpdf->font_size / $this->kf;
        // Return as pixels
        $size = $this->size_converter->convert($size, $maxsize, $fontsize, false) * 1 / (25.4 / $this->mpdf->dpi);
        return $size;
    }
    public function convert_svg_size_pts($size = 5)
    {
        // usefontsize - setfalse for e.g. margins - will ignore fontsize for % values
        // Depends of maxsize value to make % work properly. Usually maxsize == pagewidth
        // For text $maxsize = Fontsize
        // Setting e.g. margin % will use maxsize (pagewidth) and em will use fontsize
        $maxsize = $this->mpdf->font_size;
        // Return as pts
        $size = $this->size_converter->convert($size, $maxsize, false, true) * 72 / 25.4;
        return $size;
    }
    public function svg_rect(array $arguments)
    {
        if ($arguments['h'] == 0 || $arguments['w'] == 0) {
            return '';
        }
        $x = $this->convert_svg_size_pixels($arguments['x'], 'x');
        // mPDF 4.4.003
        $y = $this->convert_svg_size_pixels($arguments['y'], 'y');
        // mPDF 4.4.003
        $h = $this->convert_svg_size_pixels($arguments['h'], 'y');
        // mPDF 4.4.003
        $w = $this->convert_svg_size_pixels($arguments['w'], 'x');
        // mPDF 4.4.003
        $rx = $this->convert_svg_size_pixels($arguments['rx'], 'x');
        // mPDF 4.4.003
        $ry = $this->convert_svg_size_pixels($arguments['ry'], 'y');
        // mPDF 4.4.003
        // mPDF 4.4.003
        if ($rx > $w / 2) {
            $rx = $w / 2;
     ...
     */
    public function svg_ellipse(array $arguments)
    {
        if ($arguments['rx'] == 0 || $arguments['ry'] == 0) {
            return '';
        }
        $kappa = 4 * (sqrt(2) - 1) / 3;
        $cx = $this->convert_svg_size_pixels($arguments['cx'], 'x');
        $cy = $this->convert_svg_size_pixels($arguments['cy'], 'y');
        $rx = $this->convert_svg_size_pixels($arguments['rx'], 'x');
        $ry = $this->convert_svg_size_pixels($arguments['ry'], 'y');
        $x1 = $cx;
        $y1 = -$cy + $ry;
    ...
    public function svg_polyline($arguments, $ispolyline = true)
    {
        if ($ispolyline) {
            $xbase = $arguments[0];
            $ybase = -$arguments[1];
            ...
            // Zero length line
            $xbase = $this->convert_svg_size_pixels($arguments[0], 'x');
            $ybase = -$this->convert_svg_size_pixels($arguments[1], 'y');
        }
        ...
        for ($i = 2; $i < count($arguments); $i += 2) {
            if ($ispolyline) {
                ...
                $tmp_x = $this->convert_svg_size_pixels($arguments[$i], 'x');
                $tmp_y = -$this->convert_svg_size_pixels($arguments[$i + 1], 'y');
            }
    ...
    public function svg_polygon($arguments)
    {
        $xbase = $arguments[0];
        $ybase = -$arguments[1];
        $path_cmd = sprintf('%.3F %.3F m ', $xbase * $this->kp, $ybase * $this->kp);
    ...
    public function svg_text()
    {
        $current_style = $this->txt_style[count($this->txt_style) - 1];
        // mPDF 5.7.4
        $style = '';
        $op = '';
        $render = -1;
        if (isset($this->txt_data[2])) {
            ...
            if (isset($this->svg_font[$current_style['font-family']])) {
                ...
                $gs = $this->mpdf->add_ext_g_state(['ca' => $fopacity, 'CA' => $sopacity, 'BM' => '/Normal']);
                $this->mpdf->extgstates[$gs]['fo'] = true;
                $opacitystr = sprintf(' /GS%d gs ', $gs);
                $fillstr = '';
                if (isset($current_style['fill']) && $current_style['fill'] != 'none') {
                    $col = $this->color_converter->convert($current_style['fill'], $this->mpdf->pdfa_xwarnings);
                    $fillstr = $this->mpdf->set_f_color($col, true);
                    $render = '0';
                    // Fill (only)
                    $op = 'f';
                }
                ...
                if ($stroke_width > 0 && $current_style['stroke'] != 'none') {
                    $scol = $this->color_converter->convert($current_style['stroke'], $this->mpdf->pdfa_xwarnings);
                    if ($scol) {
                        $strokestr .= $this->mpdf->set_d_color($scol, true) . ' ';
                    }
                    $linewidth = $this->convert_svg_size_pixels($stroke_width);
                    if ($linewidth > 0) {
                ...
                while ($i < $nb) {
                    ...
                    $this->sub_path_init = true;
                    $this->path_b_box = [999999, 999999, -999999, -999999];
                    foreach ($commands as $cmd) {
                        ...
                        $subpath_cmd .= $this->svg_path($command, $arguments);
                    }
                    $subpath_cmd .= $op . ' Q ';
                    if ($this->path_b_box[2] == -1999998) {
                        $this->path_b_box[2] = 100;
                    }
                    if ($this->path_b_box[3] == -1999998) {
                        $this->path_b_box[3] = 100;
                    }
                    if ($this->path_b_box[0] == 999999) {
                        $this->path_b_box[0] = 0;
                    }
                    if ($this->path_b_box[1] == 999999) {
                        $this->path_b_box[1] = 0;
                    }
                    $sw += $horiz_adv_x;
                    $i++;
            ...
            $current_style['font-family'] = $this->mpdf->set_font($current_style['font-family'], $style, $size, false);
            $this->mpdf->current_font['fo'] = true;
            $opacitystr = '';
            // mPDF 6
            $fopacity = 1;
            if (isset($current_style['fill-opacity'])) {
            ...
            if (isset($current_style['stroke-width']) && $current_style['stroke-width'] > 0 && $current_style['stroke'] != 'none') {
                ...
                $linewidth = $this->convert_svg_size_pixels($current_style['stroke-width']);
                if ($linewidth > 0) {
            ...
            if ($this->mpdf->use_kerning) {
                if ($this->mpdf->current_font['haskernGPOS']) {
                    ...
                    $textvar = $textvar | Text_Vars::FC_KERNING;
                }
            }
            // Use OTL OpenType Table Layout - GSUB & GPOS
            if (isset($this->mpdf->current_font['useOTL']) && $this->mpdf->current_font['useOTL']) {
                $txt = $this->otl->apply_otl($txt, $this->mpdf->current_font['useOTL']);
                $ot_ldata = $this->otl->ot_ldata;
            }
            $this->mpdf->ot_ltags = $save_ot_ltags;
            $this->mpdf->magic_reverse_dir($txt, $this->mpdf->directionality, $ot_ldata);
            $this->mpdf->current_font['used'] = true;
            $sw = $this->mpdf->get_string_width($txt, true, $ot_ldata, $textvar);
            // also adds characters to subset
            // mPDF 5.7.4
            $this->textlength = $sw * 1 / (25.4 / $this->mpdf->dpi);
            $this->texttotallength += $this->textlength;
            $pdfx = $x * $this->kp;
            $pdfy = -$y * $this->kp;
            $aixextra = sprintf(' /F%d %.3F Tf %s %s Tr %s %s ', $this->mpdf->current_font['i'], $this->mpdf->font_size_pt, $opacitystr, $render, $fillstr, $strokestr);
            $path_cmd = 'q 1 0 0 1 mPDF-AXS(0.00) 0 cm ';
            // Align X-shift
            $path_cmd .= $this->mpdf->Text($pdfx, $pdfy, $txt, $ot_ldata, $textvar, $aixextra, 'SVG', true);
            $path_cmd .= " Q\n";
            unset($this->txt_data[0], $this->txt_data[1], $this->txt_data[2]);
            if (isset($current_style['font-size-parent'])) {
                $this->mpdf->set_font_size($current_style['font-size-parent']);
            }
        } else {
            return ' ';
        }
        ...
        $this->mpdf->set_font($prev_style['font-family'], $style, $size, false);
        return $path_cmd;
    }
    public function svg_define_txt_style(array $critere_style)
    {
        ...
        if (isset($critere_style['style'])) {
            ...
            // mPDF 6
            if (preg_match("/[^-]opacity:\\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m) || preg_match("/^opacity:\\s*([a-z0-9.]*|none)/i", $critere_style['style'], $m)) {
                $current_style['fill-opacity'] = $m[1];
                $current_style['stroke-opacity'] = $m[1];
            }
            $tmp = preg_replace("/(.*)fill-opacity:\\s*([a-z0-9.]*|none)(.*)/i", '$2', $critere_style['style']);
        ...
        if (isset($critere_style['font'])) {
            ...
            if ($tmp != $critere_style['font']) {
                $current_style['font-size'] = $this->convert_svg_size_pts($tmp);
                $this->mpdf->set_font('', '', $current_style['font-size'], false);
            }
        }
        // mPDF 6
        if (isset($critere_style['opacity']) && $critere_style['opacity'] != 'inherit') {
        ...
        if (isset($critere_style['font-size']) && $critere_style['font-size'] != 'inherit') {
            ...
            $current_style['font-size'] = $this->convert_svg_size_pts($critere_style['font-size']);
            $this->mpdf->set_font('', '', $current_style['font-size'], false);
        }
        if (isset($critere_style['font-family']) && $critere_style['font-family'] != 'inherit') {
            ...
            foreach ($aux_fontlist as $f) {
                ...
                if (!$this->mpdf->using_core_font && in_array($v, $this->mpdf->available_unifonts) || $this->mpdf->using_core_font && in_array($v, ['courier', 'times', 'helvetica', 'arial']) || in_array($v, ['sjis', 'uhc', 'big5', 'gb']) || isset($this->svg_font[$v][$svgfontstyle])) {
                    // mPDF 6
            ...
            if (!$found) {
                foreach ($aux_fontlist as $f) {
                    ...
                    if (in_array($v, $this->mpdf->sans_fonts) || in_array($v, $this->mpdf->serif_fonts) || in_array($v, $this->mpdf->mono_fonts) || isset($this->svg_font[$v][$svgfontstyle])) {
                        // mPDF 6
                        $current_style['font-family'] = $v;
                        break;
                    }
    ...
    public function svg_add_gradient($id, $array_gradient)
    {
        $this->svg_gradient[$id] = $array_gradient;
    }
    public function svg_write_string($content)
    {
     ...
     */
    public function merge_styles($data)
    {
        $xml = new \Dom_Document();
        if (!$xml->load_xml($data, LIBXML_NOERROR)) {
            return $data;
        }
        // Check it's an SVG
        $svg_node = $xml->get_elements_by_tag_name('svg');
        if ($svg_node->length === 0) {
            return $data;
        }
        // Find the style node
        $styles = [];
        /** @var $styleNode \DOMNode */
        foreach ($svg_node->item(0)->get_elements_by_tag_name('style') as $style_node) {
            preg_match_all('/(\.[^{]+)\s*\{\s*([^}]+)\s*}/m', $style_node->node_value, $matches, PREG_SET_ORDER);
            foreach ($matches as $css_block) {
                $css = preg_replace('/\s{2,}/', ' ', $css_block[2]);
                // Clean spaces or new lines
                $selector = trim($css_block[1]);
                $styles[$selector] = isset($styles[$css_block[1]]) ? $styles[$selector] . ' ' . $css : $css;
            }
        }
        if (empty($styles)) {
            return $data;
        ...
        $set_styles_inline = function (\Dom_Node $xml) use ($styles, &$set_styles_inline) {
            // Apply the styles to the elements
            foreach ($xml->child_nodes as $node) {
                if ($node->has_child_nodes()) {
                    $set_styles_inline($node);
                }
                ...
                $classes = explode(' ', $node->get_attribute('class'));
                foreach ($classes as $class) {
                    $class = '.' . trim($class);
                    if (isset($styles[$class])) {
                        $style = $node->has_attribute('style') ? $styles[$class] . ' ' . $node->get_attribute('style') : $styles[$class];
                        $node->set_attribute('style', $style);
                    }
                }
            }
        };
        $set_styles_inline($xml);
        return $xml->save_xml();
    }
    /**
     * analise le svg et renvoie aux fonctions precedente our le traitement
     */
    public function image_svg($data)
    {
        ...
        // mPDF 5.7.4
        $data = $this->merge_styles($data);
        if ($this->mpdf->svg_auto_font) {
            $data = $this->mark_script_to_lang($data);
        }
        $this->svg_info = [];
        // mPDF 6
        ...
        xml_set_element_handler($svg2pdf_xml_parser, [$this, 'xml_svg2pdf_start'], [$this, 'xml_svg2pdf_end']);
        xml_set_character_data_handler($svg2pdf_xml_parser, [$this, 'characterData']);
    ...
    public function mark_script_to_lang($html)
    {
        ...
        foreach ($a as $i => $e) {
            if ($i % 2 == 0) {
                $e = Utf_String::strcode2utf($e);
                $e = $this->mpdf->lesser_entity_decode($e);
                $earr = $this->mpdf->utf8string_to_array($e, false);
                $scriptblock = 0;
                $scriptblocks = [];
                $scriptblocks[0] = 0;
                $chardata = [];
                ...
                for ($sch = 0; $sch < $subchunk; $sch++) {
                    if ($scriptblocks[$sch] > 0 && $scriptblocks[$sch] != $this->mpdf->base_script && $scriptblocks[$sch + 1] == $this->mpdf->base_script) {
                ...
                for ($sch = 0; $sch <= $subchunk; $sch++) {
                    if (isset($chardata[$sch])) {
                        ...
                        for ($j = 0; $j < count($chardata[$sch]); $j++) {
                            $s .= Utf_String::code2utf($chardata[$sch][$j]['uni']);
                        }
                        ...
                        if ($scriptblocks[$sch] == Ucdn::SCRIPT_LATIN && $this->mpdf->auto_vietnamese && preg_match('/([' . $this->script_to_language->get_language_delimiters('viet') . '])/u', $s)) {
                            $lang = 'vi';
                        } elseif ($scriptblocks[$sch] == Ucdn::SCRIPT_ARABIC && $this->mpdf->auto_arabic) {
                            if (preg_match('/[' . $this->script_to_language->get_language_delimiters('sindhi') . ']/u', $s)) {
                                $lang = 'sd';
                            } elseif (preg_match('/[' . $this->script_to_language->get_language_delimiters('urdu') . ']/u', $s)) {
                                $lang = 'ur';
                            } elseif (preg_match('/[' . $this->script_to_language->get_language_delimiters('pashto') . ']/u', $s)) {
                                $lang = 'ps';
                            } elseif (preg_match('/[' . $this->script_to_language->get_language_delimiters('persian') . ']/u', $s)) {
                                $lang = 'fa';
                            } elseif ($this->mpdf->base_script != Ucdn::SCRIPT_ARABIC && $this->script_to_language->get_language_by_script($scriptblocks[$sch])) {
                                $lang = "'." . $this->script_to_language->get_language_by_script($scriptblocks[$sch]) . "'";
                            }
                        } elseif ($scriptblocks[$sch] > 0 && $scriptblocks[$sch] != $this->mpdf->base_script && $this->script_to_language->get_language_by_script($scriptblocks[$sch])) {
                            $lang = $this->script_to_language->get_language_by_script($scriptblocks[$sch]);
                        }
                        if ($lang) {
    ...
    public function xml_svg2pdf_start($parser, $name, array $attribs)
    {
        global $last_gradid, $last_svg_fontid, $last_svg_fontdefw, $last_svg_fontstyle;
        // mPDF 6
        // mPDF 6
        ...
        if (strtolower($name) == 'font-face') {
            ...
            if (isset($attribs['font-family']) && $attribs['font-family']) {
                $tmp_svg_font = ['units-per-em' => isset($attribs['units-per-em']) ? $attribs['units-per-em'] : '', 'd' => '', 'glyphs' => []];
        ...
        if (strtolower($name) == 'glyph') {
            ...
            if ($last_svg_fontid && isset($attribs['unicode'])) {
                $this->svg_font[$last_svg_fontid][$last_svg_fontstyle]['glyphs'][$attribs['unicode']] = ['horiz-adv-x' => isset($attribs['horiz-adv-x']) ? $attribs['horiz-adv-x'] : $last_svg_fontdefw, 'd' => isset($attribs['d']) ? $attribs['d'] : ''];
            }
            return;
        }
        if (strtolower($name) == 'lineargradient') {
            // mPDF 5.7.2
            $tmp_gradient = ['type' => 'linear', 'transform' => isset($attribs['gradientTransform']) ? $attribs['gradientTransform'] : '', 'units' => isset($attribs['gradientUnits']) ? $attribs['gradientUnits'] : '', 'spread' => isset($attribs['spreadMethod']) ? $attribs['spreadMethod'] : '', 'color' => []];
            if (isset($attribs['x1'])) {
            ...
            $this->svg_add_gradient($attribs['id'], $tmp_gradient);
            return;
        }
        if (strtolower($name) == 'radialgradient') {
            $tmp_gradient = ['type' => 'radial', 'transform' => isset($attribs['gradientTransform']) ? $attribs['gradientTransform'] : '', 'units' => isset($attribs['gradientUnits']) ? $attribs['gradientUnits'] : '', 'spread' => isset($attribs['spreadMethod']) ? $attribs['spreadMethod'] : '', 'color' => []];
            if (isset($attribs['cx'])) {
        ...
        if (strtolower($name) == 'stop') {
            ...
            $col = $this->color_converter->convert($color, $this->mpdf->pdfa_xwarnings);
            if (!$col) {
                $col = $this->color_converter->convert('#000000', $this->mpdf->pdfa_xwarnings);
            }
            ...
            $tmp_color = ['color' => $color_final, 'offset' => isset($attribs['offset']) ? $attribs['offset'] : '', 'opacity' => $stop_opacity];
        ...
        switch (strtolower($name)) {
            ...
            case 'path':
                ...
                foreach ($commands as $c) {
                    ...
                    $path_cmd .= $this->svg_path($command, $arguments);
                }
                if ($this->path_b_box[2] == -1999998) {
                    $this->path_b_box[2] = 100;
                }
                ...
                $path_style = $this->svg_define_style($critere_style);
                break;
            case 'rect':
                if (!isset($attribs['x'])) {
                    $attribs['x'] = 0;
                ...
                $path_cmd = $this->svg_rect($arguments);
                $critere_style = $attribs;
            ...
            case 'circle':
                ...
                $path_cmd = $this->svg_ellipse($arguments);
                $critere_style = $attribs;
            ...
            case 'line':
                ...
                $path_cmd = $this->svg_polyline($arguments, false);
                $critere_style = $attribs;
            ...
            case 'polyline':
                ...
                $path_cmd = $this->svg_polyline($arguments);
                $critere_style = $attribs;
            ...
            case 'polygon':
                ...
                $path_cmd = $this->svg_polygon($arguments);
            ...
            // no break
            case 'g':
                $array_style = $this->svg_define_style($attribs);
                if (!empty($array_style['transformations'])) {
                    ...
                    if ($this->intext) {
                        ...
                        // mPDF 5.7.4
                        $this->svg_write_string(' q ' . $array_style['transformations']);
                    }
                }
                array_push($this->svg_style, $array_style);
                $this->svg_define_txt_style($attribs);
                break;
            case 'text':
                $this->textlength = 0;
                // mPDF 5.7.4
                ...
                // mPDF 5.7.4
                $styl = '';
                if ($this->mpdf->svg_classes && isset($attribs['class']) && $attribs['class']) {
                    $classes = preg_split('/\s+/', trim($attribs['class']));
                    foreach ($classes as $class) {
                        if (isset($this->css_manager->CSS['CLASS>>' . strtoupper($class)])) {
                            $c = $this->css_manager->CSS['CLASS>>' . strtoupper($class)];
                ...
                if ($this->mpdf->svg_auto_font && isset($attribs['lang']) && $attribs['lang']) {
                    if (!$this->mpdf->using_core_font) {
                        if ($attribs['lang'] != $this->mpdf->default_lang) {
                            list($core_suitable, $mpdf_unifont) = $this->language_to_font->get_language_options($attribs['lang'], $this->mpdf->use_adobe_cjk);
                            if ($mpdf_unifont) {
                ...
                array_push($this->svg_style, $array_style);
                $this->txt_data = [];
                $x = isset($attribs['x']) ? $this->convert_svg_size_pixels($attribs['x'], 'x') : 0;
                // mPDF 5.7.4
                $y = isset($attribs['y']) ? $this->convert_svg_size_pixels($attribs['y'], 'y') : 0;
                // mPDF 5.7.4
                $x += isset($attribs['dx']) ? $this->convert_svg_size_pixels($attribs['dx'], 'x') : 0;
                // mPDF 5.7.4
                $y += isset($attribs['dy']) ? $this->convert_svg_size_pixels($attribs['dy'], 'y') : 0;
                // mPDF 5.7.4
                $this->txt_data[0] = $x;
                // mPDF 5.7.4
                $this->txt_data[1] = $y;
                ...
                $this->svg_define_txt_style($critere_style);
                $this->textanchor = $this->txt_style[count($this->txt_style) - 1]['text-anchor'];
                // mPDF 5.7.4
                $this->text_xorigin = $this->txt_data[0];
                // mPDF 5.7.4
                $this->text_yorigin = $this->txt_data[1];
                // mPDF 5.7.4
                $this->textjuststarted = true;
                // mPDF 5.7.4
                break;
            // mPDF 5.7.4
            case 'tspan':
                ...
                if (isset($attribs['x']) || isset($attribs['y'])) {
                    ...
                    $this->svg_write_string($this->textoutput);
                    $this->text_xorigin += $this->textlength;
                    $current_x = $this->text_xorigin;
                    $current_y = $this->text_yorigin;
                    $this->textlength = 0;
                    $this->texttotallength = 0;
                    $this->textoutput = '';
                    $x = isset($attribs['x']) ? $this->convert_svg_size_pixels($attribs['x'], 'x') : $current_x;
                    $y = isset($attribs['y']) ? $this->convert_svg_size_pixels($attribs['y'], 'y') : $current_y;
                    $this->txt_data[0] = $x;
                    $this->txt_data[1] = $y;
                    $critere_style = $attribs;
                    unset($critere_style['x'], $critere_style['y']);
                    ...
                    $current_x = $this->text_xorigin;
                    $current_y = $this->text_yorigin;
                    $current_x += isset($attribs['dx']) ? $this->convert_svg_size_pixels($attribs['dx'], 'x') : 0;
                    $current_y += isset($attribs['dy']) ? $this->convert_svg_size_pixels($attribs['dy'], 'y') : 0;
                    $this->txt_data[0] = $current_x;
                    $this->txt_data[1] = $current_y;
                    $critere_style = $attribs;
                    unset($critere_style['x'], $critere_style['y']);
                    $this->svg_define_txt_style($critere_style);
                    $this->text_xorigin = $current_x;
                    $this->text_yorigin = $current_y;
                }
                if (!empty($array_style['transformations'])) {
                    $this->textoutput .= ' q ' . $array_style['transformations'];
                }
        ...
        if (isset($path_cmd) && $path_cmd) {
            // mPDF 5.0
            list($prestyle, $poststyle) = $this->svg_style($path_style, $attribs, strtolower($name));
            if (isset($path_style['transformations']) && $path_style['transformations']) {
                // transformation on an element
                $this->svg_write_string(' q ' . $path_style['transformations'] . $prestyle . $path_cmd . $poststyle . " Q\n");
            } else {
                $this->svg_write_string(' q ' . $prestyle . $path_cmd . $poststyle . " Q\n");
                // mPDF 5.7.4
            }
        }
    }
    public function character_data($parser, $data)
    {
        ...
        if (isset($this->txt_data[2])) {
            ...
            $this->txt_data[2] = $data;
            $this->txt_data[0] = $this->text_xorigin;
            $this->txt_data[1] = $this->text_yorigin;
        }
    }
        ...
        switch ($name) {
            ...
            case 'text':
                if (!empty($this->txt_data[2])) {
                    $this->txt_data[2] = rtrim($this->txt_data[2]);
                    // mPDF 5.7.4
                }
                $path_cmd = $this->svg_text();
                $this->textoutput .= $path_cmd;
                // mPDF 5.7.4
                $tmp = count($this->svg_style) - 1;
                $current_style = $this->svg_style[$tmp];
                if (!empty($current_style['transformations'])) {
    ...
    private function compute_bezier_bounding_box(array $start, array $c)
    {
        $P0 = [$start[0], $start[1]];
        $P1 = [$c[0], $c[1]];
        $P2 = [$c[2], $c[3]];
        ...
        for ($i = 0; $i <= 1; $i++) {
            ...
            if ($a == 0) {
                ...
                $t = -$c / $b;
                if ($t > 0 && $t < 1) {
                    $bounds[$i][] = (1 - $t) ** 3 * $P0[$i] + 3 * (1 - $t) ** 2 * $t * $P1[$i] + 3 * (1 - $t) * $t ** 2 * $P2[$i] + $t ** 3 * $P3[$i];
                }
                continue;
            }
            $b2ac = $b ** 2 - 4 * $c * $a;
            ...
            $t1 = (-$b + sqrt($b2ac)) / (2 * $a);
            if ($t1 > 0 && $t1 < 1) {
                $bounds[$i][] = (1 - $t1) ** 3 * $P0[$i] + 3 * (1 - $t1) ** 2 * $t1 * $P1[$i] + 3 * (1 - $t1) * $t1 ** 2 * $P2[$i] + $t1 ** 3 * $P3[$i];
            }
            $t2 = (-$b - sqrt($b2ac)) / (2 * $a);
            if ($t2 > 0 && $t2 < 1) {
                $bounds[$i][] = (1 - $t2) ** 3 * $P0[$i] + 3 * (1 - $t2) ** 2 * $t2 * $P1[$i] + 3 * (1 - $t2) * $t2 ** 2 * $P2[$i] + $t2 ** 3 * $P3[$i];
            }
        }
        $x = min($bounds[0]);
        $x2 = max($bounds[0]);
    ...
    private function test_intersect_circle($cx, $cy, $cr)
    {
        ...
        if ($cx >= 0 && $cx <= 1 && $cy >= 0 && $cy <= 1) {
            ...
            $d1 = sqrt(($cy - 0) ** 2 + ($cx - 0) ** 2);
            $d2 = sqrt(($cy - 1) ** 2 + ($cx - 0) ** 2);
            $d3 = sqrt(($cy - 0) ** 2 + ($cx - 1) ** 2);
            $d4 = sqrt(($cy - 1) ** 2 + ($cx - 1) ** 2);
            $maxd = max($d1, $d2, $d3, $d4);
        }
        return $cr < $maxd;
    }
    private function test_intersect($x1, $y1, $x2, $y2, $x3, $y3, $x4, $y4)
    {
