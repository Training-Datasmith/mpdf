    ...
    public function write_images()
    {
        ...
        foreach ($this->mpdf->images as $file => $info) {
            ...
            if (isset($info['interpolation']) && $info['interpolation']) {
                $this->writer->write('/Interpolate true');
                // mPDF 6 - image interpolation shall be performed by a conforming reader
            }
            ...
            if (isset($info['icc']) && $info['icc'] !== false) {
                ...
                if ($this->mpdf->PDFX || $this->mpdf->PDFA && $this->mpdf->restrict_color_space === 3) {
                    throw new \Mpdf\Mpdf_Exception('PDFA1-b and PDFX/1-a files do not permit using mixed colour space (' . $file . ').');
                }
                ...
                if ($info['cs'] === 'DeviceCMYK') {
                    if ($this->mpdf->PDFA && $this->mpdf->restrict_color_space !== 3) {
                        throw new \Mpdf\Mpdf_Exception('PDFA1-b does not permit Images using mixed colour space (' . $file . ').');
                    }
                    if ($info['type'] === 'jpg') {
                        $this->writer->write('/Decode [1 0 1 0 1 0 1 0]');
                    }
                } elseif (($this->mpdf->PDFX || $this->mpdf->PDFA && $this->mpdf->restrict_color_space === 3) && $info['cs'] === 'DeviceRGB') {
                    throw new \Mpdf\Mpdf_Exception('PDFA1-b and PDFX/1-a files do not permit using mixed colour space (' . $file . ').');
                }
            }
