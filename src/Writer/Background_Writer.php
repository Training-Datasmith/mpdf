...
final class Background_Writer
{
    ...
    public function write_patterns()
    {
        $pattern_count = count($this->mpdf->patterns);
        for ($i = 1; $i <= $pattern_count; $i++) {
            ...
            if (!$img_obj) {
                throw new \Mpdf\Mpdf_Exception('Problem: Image object not found for background pattern ' . $img['i']);
            }
            $this->writer->object();
    ...
    public function write_shaders()
    {
        $maxid = count($this->mpdf->gradients);
        ...
        foreach ($this->mpdf->gradients as $id => $grad) {
            ...
            if (empty($grad['is_mask'])) {
                ...
                if (isset($grad['colorspace'])) {
                    $this->writer->write('/ColorSpace /Device' . $grad['colorspace']);
                    // Can use CMYK if all C0 and C1 above have 4 values
                } else {
            ...
            if (isset($grad['trans']) && $grad['trans']) {
                ...
                $str = 'q /a0 gs /Pattern cs /p' . $transid . ' scn -' . $this->mpdf->w_pt / 2 . ' -' . $this->mpdf->h_pt / 2 . ' ' . 2 * $this->mpdf->w_pt . ' ' . 2 * $this->mpdf->h_pt . ' re f Q';
                ...
                $this->writer->write('/BBox [-' . $this->mpdf->w_pt / 2 . ' -' . $this->mpdf->h_pt / 2 . ' ' . 2 * $this->mpdf->w_pt . ' ' . 2 * $this->mpdf->h_pt . ']');
