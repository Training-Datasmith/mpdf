...
class Barcode
{
    public function get_barcode_array($code, $type, $pr = '', $quiet_zone_left = null, $quiet_zone_right = null)
    {
        $barcode = $this->get_barcode($code, $type, $pr, $quiet_zone_left, $quiet_zone_right);
        return $barcode ? $barcode->get_data() : false;
    }
    public function get_checksum($code, $type)
    {
        $barcode = $this->get_barcode($code, $type);
        return $barcode ? $barcode->get_checksum() : false;
    }
    /**
     * @param string $code
     * @param string $type
     ...
     */
    public function get_barcode($code, $type, $pr = 0.0, $quiet_zone_left = null, $quiet_zone_right = null)
    {
        switch (strtoupper($type)) {
            ...
            case 'EAN13':
                // EAN 13
                return new Barcode\Ean_Upc($code, 13, 11, 7, 0.33, 25.93);
            case 'UPCA':
                // UPC-A
                return new Barcode\Ean_Upc($code, 12, 9, 9, 0.33, 25.91);
            case 'UPCE':
                // UPC-E
                return new Barcode\Ean_Upc($code, 6, 9, 7, 0.33, 25.93);
            case 'EAN8':
                // EAN 8
                return new Barcode\Ean_Upc($code, 8, 7, 7, 0.33, 21.64);
            case 'EAN2':
                // 2-Digits UPC-Based Extention
                return new Barcode\Ean_Ext($code, 2, 7, 7, 0.33, 20, 9);
            case 'EAN5':
                // 5-Digits UPC-Based Extention
                return new Barcode\Ean_Ext($code, 5, 7, 7, 0.33, 20, 9);
            case 'IMB':
                // IMB - Intelligent Mail Barcode - Onecode - USPS-B-3200
                $xdim = 0.508;
                // Nominal value for X-dim (bar width) in mm (spec.)
                $bpi = 22;
                // Bars per inch
            ...
            case 'C39':
                ...
                return new Barcode\Code39($this->sanitize_code($code), $pr > 0 ? $pr : 2.5, false, false, $quiet_zone_left, $quiet_zone_right);
            case 'C39+':
                // CODE 39 with checksum
                return new Barcode\Code39($this->sanitize_code($code), $pr > 0 ? $pr : 2.5, false, true, $quiet_zone_left, $quiet_zone_right);
            case 'C39E':
                // CODE 39 EXTENDED
                return new Barcode\Code39($this->sanitize_code($code), $pr > 0 ? $pr : 2.5, true, false, $quiet_zone_left, $quiet_zone_right);
            case 'C39E+':
                // CODE 39 EXTENDED + CHECKSUM
                return new Barcode\Code39($this->sanitize_code($code), $pr > 0 ? $pr : 2.5, true, true, $quiet_zone_left, $quiet_zone_right);
            case 'S25':
                // Standard 2 of 5
    ...
    private function sanitize_code($code)
    {
        ...
        // mPDF 5.3.95  (for utf-8 encoded)
        $code = str_replace(chr(160), ' ', $code);
        // mPDF 5.3.95	(for win-1252)
        return $code;
    }
}