...
class Arrays
{
     ...
     * @return array
     */
    public static function all_unique_sorted_combinations($array, $max_size = null)
    {
    ...
    public static function combinations($array, $k)
    {
        ...
        do {
            ...
            $another_combination = false;
            $reset_from_index = -1;
            for ($i = $k - 1; $i >= 0; $i--) {
                if ($indexes[$i] < $max_indexes[$i]) {
                    $indexes[$i]++;
                    $another_combination = true;
                    break;
                }
                $reset_from_index = $i;
            }
            if ($reset_from_index > 0) {
                for ($i = $reset_from_index; $i < $k; $i++) {
                    $indexes[$i] = $indexes[$i - 1] + 1;
                }
            }
        } while ($another_combination);
        return $combinations;
    }
    /**
     ...
     */
    public static function unique_recursive_merge(...$arrays)
    {
        ...
        foreach ($arrays as $array) {
            foreach ($array as $key => $value) {
                if ((string) $key === (string) (int) $key) {
                    ...
                    $results[$key] = self::unique_recursive_merge($results[$key], $value);
                } else {
                    $results[$key] = $value;
                }
            }
