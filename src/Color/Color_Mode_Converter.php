...
class Color_Mode_Converter
{
     ...
     * @return float[]
     */
    public function rgb2gray(array $c)
    {
        if (isset($c[4])) {
     ...
     * @return float[]
     */
    public function rgb2cmyk(array $c)
    {
        $cyan = 1 - $c[1] / 255;
        $magenta = 1 - $c[2] / 255;
        $yellow = 1 - $c[3] / 255;
     ...
     * @return float[]
     */
    public function cmyk2rgb(array $c)
    {
        $rgb = [];
        $colors = 255 - $c[4] * 2.55;
    ...
    public function rgb2hsl($r, $g, $b)
    {
        ...
        if ($diff == 0) {
            ...
            $r_diff = (($max - $r) / 6 + $diff / 2) / $diff;
            $g_diff = (($max - $g) / 6 + $diff / 2) / $diff;
            $b_diff = (($max - $b) / 6 + $diff / 2) / $diff;
            if ($r == $max) {
                $h = $b_diff - $g_diff;
            } elseif ($g == $max) {
                $h = 1 / 3 + $r_diff - $b_diff;
