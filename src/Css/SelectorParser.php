...
class Selector_Parser
{
     ...
     */
    public function parse_page_selector(array $tags)
    {
        $level = count($tags);
        $t = '';
        $t2 = '';
     ...
     */
    public function parse_simple_selector(array $tags)
    {
        ...
        if (preg_match('/^[.](.*)$/', $t, $m)) {
            ...
            $tag = 'LANG>>' . strtolower($m[1]);
        } elseif (preg_match('/^(' . $this->mpdf->allowed_cs_stags . ')[.](.*)$/', $t, $m)) {
            // mPDF 6  Special case for lang as attribute selector
            $classes = explode('.', $m[2]);
            sort($classes);
            $tag = $m[1] . '>>CLASS>>' . implode('.', $classes);
        } elseif (preg_match('/^(' . $this->mpdf->allowed_cs_stags . ')\s*:NTH-CHILD\((.*)\)$/', $t, $m)) {
            $tag = $m[1] . '>>SELECTORNTHCHILD>>' . $m[2];
        } elseif (preg_match('/^(' . $this->mpdf->allowed_cs_stags . ')[#](.*)$/', $t, $m)) {
            $tag = $m[1] . '>>ID>>' . $m[2];
        } elseif (preg_match('/^(' . $this->mpdf->allowed_cs_stags . ')\[LANG=[\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\]$/', $t, $m)) {
            $tag = $m[1] . '>>LANG>>' . strtolower($m[2]);
        } elseif (preg_match('/^(' . $this->mpdf->allowed_cs_stags . '):LANG\([\'\"]{0,1}([A-Z\-]{2,11})[\'\"]{0,1}\)$/', $t, $m)) {
            // mPDF 6  Special case for lang as attribute selector
            $tag = $m[1] . '>>LANG>>' . strtolower($m[2]);
        } elseif (preg_match('/^(' . $this->mpdf->allowed_cs_stags . ')$/', $t)) {
            // mPDF 6  Special case for lang as attribute selector
            $tag = $t;
        }
        return $tag ?: null;
     ...
     */
    public function parse_cascaded_selector(array $tags)
    {
        $tmp = [];
        $level = count($tags);
        for ($n = 0; $n < $level; $n++) {
     ...
     */
    public function matches_nth_child(array $nth_components, $index)
    {
        ++$index;
        $select = false;
        $num_of_components = count($nth_components);
        if ($nth_components[0] === 'ODD') {
            $a = 2;
            $b = 1;
        } elseif ($nth_components[0] === 'EVEN') {
            $a = 2;
            $b = 0;
        } elseif ($num_of_components === 2) {
            $a = 0;
            $b = $nth_components[1] + 0;
        } elseif ($num_of_components === 3) {
            // e.g. (2N)
            if ($nth_components[2] === '') {
                $a = 1;
            } elseif ($nth_components[2] === '-') {
                $a = -1;
            } else {
                $a = $nth_components[2] + 0;
            }
            $b = 0;
        } elseif ($num_of_components === 4) {
            // e.g. (2N+6)
            if ($nth_components[2] === '') {
                $a = 1;
            } elseif ($nth_components[2] === '-') {
