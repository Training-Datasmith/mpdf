...
class Inline_Property_Converter
{
    /**
     * @var ColorConverter
     */
    private $color_converter;
    public function __construct(Color_Converter $color_converter)
    {
     ...
     */
    public function convert(array $properties)
    {
        $css = [];
        ...
        if (!empty($properties['colorarray'])) {
            $css['COLOR'] = $this->color_converter->col_ato_string($properties['colorarray']);
        }
        ...
        if (!empty($properties['textparam'])) {
            ...
            if (!empty($properties['textparam']['outline-COLOR'])) {
                $css['TEXT-OUTLINE-COLOR'] = $this->color_converter->col_ato_string($properties['textparam']['outline-COLOR']);
            }
        ...
        if (!empty($properties['textvar'])) {
            ...
            if ($properties['textvar'] & Text_Vars::FD_LINETHROUGH) {
                if ($properties['textvar'] & Text_Vars::FD_UNDERLINE) {
                    $css['TEXT-DECORATION'] = 'underline line-through';
                } else {
                    $css['TEXT-DECORATION'] = 'line-through';
                }
            } elseif ($properties['textvar'] & Text_Vars::FD_UNDERLINE) {
                $css['TEXT-DECORATION'] = 'underline';
            } else {
                $css['TEXT-DECORATION'] = 'none';
            }
            if ($properties['textvar'] & Text_Vars::FA_SUPERSCRIPT) {
                $css['VERTICAL-ALIGN'] = 'super';
            } elseif ($properties['textvar'] & Text_Vars::FA_SUBSCRIPT) {
                $css['VERTICAL-ALIGN'] = 'sub';
            } else {
                $css['VERTICAL-ALIGN'] = 'baseline';
            }
            if ($properties['textvar'] & Text_Vars::FT_CAPITALIZE) {
                $css['TEXT-TRANSFORM'] = 'capitalize';
            } elseif ($properties['textvar'] & Text_Vars::FT_UPPERCASE) {
                $css['TEXT-TRANSFORM'] = 'uppercase';
            } elseif ($properties['textvar'] & Text_Vars::FT_LOWERCASE) {
                $css['TEXT-TRANSFORM'] = 'lowercase';
            } else {
                $css['TEXT-TRANSFORM'] = 'none';
            }
            if ($properties['textvar'] & Text_Vars::FC_KERNING) {
            ...
            if ($properties['textvar'] & Text_Vars::FC_SMALLCAPS) {
        ...
        if (!empty($properties['OTLtags'])) {
            ...
            if (!empty($properties['OTLtags']['FFPlus'])) {
                // May contain numeric value e.g. salt4
                $f = preg_split('/\s+/', trim($properties['OTLtags']['FFPlus']));
                foreach ($f as $ff) {
                    if (strlen($ff) > 4) {
                        $font_feature[] = "'" . substr($ff, 0, 4) . "' " . substr($ff, 4);
                    } else {
                        $font_feature[] = "'" . $ff . "' 1";
                    }
                }
            }
            $css['FONT-FEATURE-SETTINGS'] = implode(', ', $font_feature);
        }
        return $css;
    }
}