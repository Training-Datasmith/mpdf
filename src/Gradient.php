...
class Gradient
{
     ...
     */
    private $writer;
    public function __construct(Mpdf $mpdf, Size_Converter $size_converter, Color_Converter $color_converter, Base_Writer $writer)
    {
        $this->mpdf = $mpdf;
    ...
    // mPDF 5.3.A1
    public function coons_patch_mesh($x, $y, $w, $h, $patch_array = [], $x_min = 0, $x_max = 1, $y_min = 0, $y_max = 1, $colspace = 'RGB', $return = false)
    {
        $s = ' q ';
    ...
    public function Gradient($x, $y, $w, $h, $type, $stops = [], $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
    {
        ...
        if ($coords[0] !== false && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $coords[0], $m)) {
            $tmp = $this->size_converter->convert($m[1], $this->mpdf->w, $this->mpdf->font_size, false);
            if ($tmp) {
                $coords[0] = $tmp / $w;
            }
        }
    ...
    private function parse_moz_linear_gradient(array $m, $repeat)
    {
        $g = [];
        ...
        if (preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i', $bgr[0]) && !preg_match('/(<#|rgb|rgba|hsl|hsla)/i', $bgr[0])) {
            ...
            $check = $this->color_converter->convert($first[0], $this->mpdf->pdfa_xwarnings);
            $start_stops = 1;
            if ($check) {
                $start_stops = 0;
            }
        ...
        $g['stops'] = [];
        for ($i = $start_stops; $i < count($bgr); $i++) {
            // parse stops
            $el = preg_split('/\s+/', trim($bgr[$i]));
            // mPDF 5.3.74
            $col = $this->color_converter->convert($el[0], $this->mpdf->pdfa_xwarnings);
            if (!$col) {
                $col = $this->color_converter->convert(255, $this->mpdf->pdfa_xwarnings);
            }
            if ($col[0] == 1) {
                $g['colorspace'] = 'Gray';
            ...
            $g['stops'][] = $this->get_stop($col, $el, true);
        }
        return $g;
    }
    private function parse_moz_radial_gradient(array $m, $repeat)
    {
        $g = [];
        ...
        $shape_size = false;
        $first = preg_split('/\s+/', trim($bgr[0]));
        $check_col = $this->color_converter->convert($first[0], $this->mpdf->pdfa_xwarnings);
        if (preg_match('/(left|center|right|bottom|top|deg|grad|rad)/i', $bgr[0]) && !preg_match('/(<#|rgb|rgba|hsl|hsla)/i', $bgr[0])) {
            $start_stops = 1;
            $pos_angle = $bgr[0];
        } elseif (trim($first[count($first) - 1]) === '0') {
        ...
        for ($i = $start_stops; $i < count($bgr); $i++) {
            ...
            $g['stops'][] = $this->get_stop($col, $el);
        }
        return $g;
    }
    private function get_stop($col, $el, $convert_offset = false)
    {
        $stop = ['col' => $col];
        if ($col[0] == 5) {
    ...
    public function parse_moz_gradient($bg)
    {
        ...
        if (preg_match('/linear-gradient\((.*)\)/', $bg, $m)) {
            $g = $this->parse_moz_linear_gradient($m, $repeat);
            if (count($g['stops'])) {
                return $g;
            }
        } elseif (preg_match('/radial-gradient\((.*)\)/', $bg, $m)) {
            $g = $this->parse_moz_radial_gradient($m, $repeat);
            if (count($g['stops'])) {
                return $g;
            }
        }
    ...
    public function parse_background_gradient($bg)
    {
        ...
        if ($count_bgr > 6) {
            ...
            $g['colorspace'] = 'RGB';
            // mPDF 5.3.74
            $cor = $this->color_converter->convert($bgr[1], $this->mpdf->pdfa_xwarnings);
            if ($cor[0] == 1) {
                $g['colorspace'] = 'Gray';
            ...
            if ($cor) {
                ...
                $g['col'] = $this->color_converter->convert(255, $this->mpdf->pdfa_xwarnings);
            }
            $cor = $this->color_converter->convert($bgr[2], $this->mpdf->pdfa_xwarnings);
            if ($cor) {
                $g['col2'] = $cor;
            } else {
                $g['col2'] = $this->color_converter->convert(255, $this->mpdf->pdfa_xwarnings);
            }
            $g['extend'] = ['true', 'true'];
