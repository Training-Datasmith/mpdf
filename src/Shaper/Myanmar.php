...
class Myanmar
{
    ...
    // myanmar_category
    const OT_DB = 3;
    // same as Indic::OT_N; /* Dot below */
    const OT_GB = 12;
    // same as Indic::OT_DOTTEDCIRCLE;
    const OT_AS = 18;
    /* Asat */
    const OT_D = 19;
    /* Digits except zero */
    ...
    public static function reordering(&$info, $gsu_bdata, $broken_syllables, $dottedcircle)
    {
        ...
        for ($i = 1; $i < $count; $i++) {
            if ($last_syllable != $info[$i]['syllable']) {
                self::reordering_syllable($info, $gsu_bdata, $last, $i);
                $last = $i;
                $last_syllable = $info[$last]['syllable'];
            }
        }
        self::reordering_syllable($info, $gsu_bdata, $last, $count);
    }
    ...
    public static function reordering_syllable(&$info, $gsu_bdata, $start, $end)
    {
        ...
        $has_reph = false;
        $limit = $start;
        if ($start + 3 <= $end && $info[$start]['myanmar_category'] == self::OT_RA && $info[$start + 1]['myanmar_category'] == self::OT_AS && $info[$start + 2]['myanmar_category'] == self::OT_H) {
            $limit += 3;
            $base = $start;
            $has_reph = true;
        }
