...
class Indic
{
    ...
    public static function set_indic_properties(&$info, $scriptblock)
    {
        ...
        if (self::in_range($u, 0x951, 0x954)) {
            $cat = self::OT_VD;
        }
        if ($u == 0x200c) {
            $cat = self::OT_ZWNJ;
    ...
    public static function initial_reordering(&$info, $gsu_bdata, $broken_syllables, $indic_config, $scriptblock, $is_old_spec, $dottedcircle)
    {
        self::update_consonant_positions($info, $gsu_bdata);
        ...
        for ($i = 1; $i < $count; $i++) {
            if ($last_syllable != $info[$i]['syllable']) {
                self::initial_reordering_syllable($info, $gsu_bdata, $indic_config, $scriptblock, $is_old_spec, $last, $i);
                $last = $i;
                $last_syllable = $info[$last]['syllable'];
            }
        }
        self::initial_reordering_syllable($info, $gsu_bdata, $indic_config, $scriptblock, $is_old_spec, $last, $count);
    }
    public static function update_consonant_positions(&$info, $gsu_bdata)
    {
        $count = count($info);
        for ($i = 0; $i < $count; $i++) {
            if ($info[$i]['indic_position'] == self::POS_BASE_C) {
                $c = $info[$i]['uni'];
                // If would substitute...
                if (isset($gsu_bdata['pref'][$c])) {
                    $info[$i]['indic_position'] = self::POS_POST_C;
                } elseif (isset($gsu_bdata['blwf'][$c])) {
                    $info[$i]['indic_position'] = self::POS_BELOW_C;
                } elseif (isset($gsu_bdata['pstf'][$c])) {
                    $info[$i]['indic_position'] = self::POS_POST_C;
                }
            }
        }
    ...
    public static function insert_dotted_circles(&$info, $dottedcircle)
    {
        ...
        if (isset($info[$idx])) {
            throw new \Mpdf\Mpdf_Exception('Unexpected error occurred in Indic processing');
        }
    ...
    public static function initial_reordering_syllable(&$info, $gsu_bdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
    {
        ...
        $syllable_type = $info[$start]['syllable'] & 0xf;
        ...
        if ($scriptblock != Ucdn::SCRIPT_KHMER) {
             ...
             * 	and has more than one consonant, Ra is excluded from candidates for
             * 	base consonants. */
            if (count($gsu_bdata['rphf']) && $start + 3 <= $end && ($indic_config[4] == self::REPH_MODE_IMPLICIT && !self::is_joiner($info[$start + 2]) || $indic_config[4] == self::REPH_MODE_EXPLICIT && $info[$start + 2]['indic_category'] == self::OT_ZWJ)) {
                /* See if it matches the 'rphf' feature. */
                //$glyphs = array($info[$start]['uni'], $info[$start + 1]['uni']);
                //if ($indic_plan->rphf->would_substitute ($glyphs, count($glyphs), true, face)) {
                if (isset($gsu_bdata['rphf'][$info[$start]['uni']]) && self::is_halant_or_coeng($info[$start + 1])) {
                    $limit += 2;
        ...
        */
                    ...
                    for ($i = $start; $i < $base - 2; $i++) {
                        if (self::is_consonant($info[$i])) {
                            if (self::is_halant_or_coeng($info[$i + 1]) && self::is_ra($info[$i + 2]['uni'])) {
                                ...
                                if (isset($gsu_bdata['blwf'][$info[$i + 2]['uni']])) {
        ...
        if ($is_old_spec && $scriptblock == Ucdn::SCRIPT_DEVANAGARI) {
            ...
            for ($i = $start; $i + 1 < $base; $i++) {
                if ($info[$i]['indic_category'] == self::OT_RA && $info[$i + 1]['indic_category'] == self::OT_H && ($i + 2 == $base || $info[$i + 2]['indic_category'] != self::OT_ZWJ)) {
        ...
        if ($scriptblock != Ucdn::SCRIPT_KHMER) {
            if (count($gsu_bdata['pref']) && $base + 2 < $end) {
                /* Find a Halant,Ra sequence and mark it for pre-base processing. */
                for ($i = $base + 1; $i + 1 < $end; $i++) {
                    // If old_spec find Ra-Halant...
                    if (isset($gsu_bdata['pref'][$info[$i + 1]['uni']]) && self::is_halant_or_coeng($info[$i]) && self::is_ra($info[$i + 1]['uni']) || $is_old_spec && isset($gsu_bdata['pref'][$info[$i]['uni']]) && self::is_halant_or_coeng($info[$i + 1]) && self::is_ra($info[$i]['uni'])) {
                        $info[$i++]['mask'] |= self::FLAG(self::PREF);
                        $info[$i++]['mask'] |= self::FLAG(self::PREF);
                        break;
                    }
    ...
    public static function final_reordering(&$info, $gsu_bdata, $indic_config, $scriptblock, $is_old_spec)
    {
        $count = count($info);
        if (!$count) {
            return;
        ...
        for ($i = 1; $i < $count; $i++) {
            if ($last_syllable != $info[$i]['syllable']) {
                self::final_reordering_syllable($info, $gsu_bdata, $indic_config, $scriptblock, $is_old_spec, $last, $i);
                $last = $i;
                $last_syllable = $info[$last]['syllable'];
            }
        }
        self::final_reordering_syllable($info, $gsu_bdata, $indic_config, $scriptblock, $is_old_spec, $last, $count);
    }
    public static function final_reordering_syllable(&$info, $gsu_bdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
    {
        /* 4. Final reordering:
         *
         ...
         * shouldn't move. */
        if ($start + 1 < $end && $info[$start]['indic_position'] == self::POS_RA_TO_BECOME_REPH && $info[$start + 1]['indic_position'] != self::POS_RA_TO_BECOME_REPH) {
            ...
            if ($reph_pos == self::REPH_POS_AFTER_SUB && !$skip_to_reph_move && !$skip_to_reph_step_5) {
                $new_reph_pos = $base;
                while ($new_reph_pos < $end && isset($info[$new_reph_pos + 1]['indic_position']) && !(self::FLAG($info[$new_reph_pos + 1]['indic_position']) & (self::FLAG(self::POS_POST_C) | self::FLAG(self::POS_AFTER_POST) | self::FLAG(self::POS_SMVD)))) {
                    $new_reph_pos++;
                }
                if ($new_reph_pos < $end) {
                    $skip_to_reph_move = true;
         ...
         * 	the following rules:
         */
        if (count($gsu_bdata['pref']) && $base + 1 < $end) {
    ...
    */
    ...
    {
        if (0x900 <= $u && $u <= 0xdff) {
            return self::$indic_table[$u - 0x900 + 0];
    ...
    {
        return self::IN_HALF_BLOCK($u, 0x900);
    }
    public static function IS_BENG($u)
    {
        return self::IN_HALF_BLOCK($u, 0x980);
    }
    public static function IS_GURU($u)
    {
        return self::IN_HALF_BLOCK($u, 0xa00);
    }
    public static function IS_GUJR($u)
    {
        return self::IN_HALF_BLOCK($u, 0xa80);
    }
    public static function IS_ORYA($u)
    {
        return self::IN_HALF_BLOCK($u, 0xb00);
    }
    public static function IS_TAML($u)
    {
        return self::IN_HALF_BLOCK($u, 0xb80);
    }
    public static function IS_TELU($u)
    {
        return self::IN_HALF_BLOCK($u, 0xc00);
    }
    public static function IS_KNDA($u)
    {
        return self::IN_HALF_BLOCK($u, 0xc80);
    }
    public static function IS_MLYM($u)
    {
        return self::IN_HALF_BLOCK($u, 0xd00);
    }
    public static function IS_SINH($u)
    {
        return self::IN_HALF_BLOCK($u, 0xd80);
    }
    ...
    {
        return self::IS_DEVA($u) ? self::POS_AFTER_SUB : (self::IS_BENG($u) ? self::POS_AFTER_POST : (self::IS_GURU($u) ? self::POS_AFTER_POST : (self::IS_GUJR($u) ? self::POS_AFTER_POST : (self::IS_ORYA($u) ? self::POS_AFTER_POST : (self::IS_TAML($u) ? self::POS_AFTER_POST : (self::IS_TELU($u) ? $u <= 0xc42 ? self::POS_BEFORE_SUB : self::POS_AFTER_SUB : (self::IS_KNDA($u) ? $u < 0xcc3 || $u > 0xcd6 ? self::POS_BEFORE_SUB : self::POS_AFTER_SUB : (self::IS_MLYM($u) ? self::POS_AFTER_POST : (self::IS_SINH($u) ? self::POS_AFTER_SUB : (self::IS_KHMR($u) ? self::POS_AFTER_POST : self::POS_AFTER_SUB))))))))));
        /* default */
    }
    public static function MATRA_POS_TOP($u)
    {
        return self::IS_DEVA($u) ? self::POS_AFTER_SUB : (self::IS_GURU($u) ? self::POS_AFTER_POST : (self::IS_GUJR($u) ? self::POS_AFTER_SUB : (self::IS_ORYA($u) ? self::POS_AFTER_MAIN : (self::IS_TAML($u) ? self::POS_AFTER_SUB : (self::IS_TELU($u) ? self::POS_BEFORE_SUB : (self::IS_KNDA($u) ? self::POS_BEFORE_SUB : (self::IS_SINH($u) ? self::POS_AFTER_SUB : (self::IS_KHMR($u) ? self::POS_AFTER_POST : self::POS_AFTER_SUB))))))));
        /* default */
    }
    public static function MATRA_POS_BOTTOM($u)
    {
        return self::IS_DEVA($u) ? self::POS_AFTER_SUB : (self::IS_BENG($u) ? self::POS_AFTER_SUB : (self::IS_GURU($u) ? self::POS_AFTER_POST : (self::IS_GUJR($u) ? self::POS_AFTER_POST : (self::IS_ORYA($u) ? self::POS_AFTER_SUB : (self::IS_TAML($u) ? self::POS_AFTER_POST : (self::IS_TELU($u) ? self::POS_BEFORE_SUB : (self::IS_KNDA($u) ? self::POS_BEFORE_SUB : (self::IS_MLYM($u) ? self::POS_AFTER_POST : (self::IS_SINH($u) ? self::POS_AFTER_SUB : (self::IS_KHMR($u) ? self::POS_AFTER_POST : self::POS_AFTER_SUB))))))))));
        /* default */
    }
