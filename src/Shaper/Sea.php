...
class Sea
{
    ...
    # Anusvara
    const OT_GB = 12;
    # Generic Base	(OT_DOTTEDCIRCLE in Indic)
    const OT_CM = 17;
    # Consonant Medial
    const OT_MR = 22;
    # Medial Ra
    ...
    public static $sea_category_char = ['x', 'C', 'V', 'T', 'H', 'x', 'x', 'x', 'x', 'x', 'A', 'x', 'G', 'x', 'x', 'x', 'x', 'M', 'x', 'x', 'x', 'x', 'R', 'x', 'x', 'x', 'a', 'b', 'p', 't'];
    ...
    public static function initial_reordering(&$info, $gsu_bdata, $broken_syllables, $scriptblock, $dottedcircle)
    {
        ...
        for ($i = 1; $i < $count; $i++) {
            if ($last_syllable != $info[$i]['syllable']) {
                self::initial_reordering_syllable($info, $gsu_bdata, $scriptblock, $last, $i);
                $last = $i;
                $last_syllable = $info[$last]['syllable'];
            }
        }
        self::initial_reordering_syllable($info, $gsu_bdata, $scriptblock, $last, $count);
    }
    ...
    public static function initial_reordering_syllable(&$info, $gsu_bdata, $scriptblock, $start, $end)
    {
    ...
    public static function final_reordering(&$info, $gsu_bdata, $scriptblock)
    {
        $count = count($info);
        if (!$count) {
            return;
        ...
        for ($i = 1; $i < $count; $i++) {
            if ($last_syllable != $info[$i]['syllable']) {
                self::final_reordering_syllable($info, $gsu_bdata, $scriptblock, $last, $i);
                $last = $i;
                $last_syllable = $info[$last]['syllable'];
            }
        }
        self::final_reordering_syllable($info, $gsu_bdata, $scriptblock, $last, $count);
    }
    public static function final_reordering_syllable(&$info, $gsu_bdata, $scriptblock, $start, $end)
    {
        /*
