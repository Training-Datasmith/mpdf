...
class Size_Converter implements \Psr\Log\Logger_Aware_Interface
{
     ...
     */
    private $mpdf;
    public function __construct($dpi, $default_font_size, Mpdf $mpdf, Logger_Interface $logger)
    {
        $this->dpi = $dpi;
    ...
    public function convert($size = 5, $maxsize = 0, $fontsize = false, $usefontsize = true)
    {
        ...
        if (!$res) {
            // ignore definition
            $this->logger->warning(sprintf('Invalid size representation "%s"', $size), ['context' => Log_Context::CSS_SIZE_CONVERSION]);
        }
        ...
        switch ($unit) {
            ...
            case 'ex':
                ...
                $size *= $this->multiply_font_size($fontsize, $maxsize, 0.5);
                break;
            case 'em':
                $size *= $this->multiply_font_size($fontsize, $maxsize, 1);
                break;
            case 'thin':
            ...
            case 'thick':
                ...
                break;
            case 'xx-small':
                $size *= $this->multiply_font_size($fontsize, $maxsize, 0.7);
                break;
            case 'x-small':
                $size *= $this->multiply_font_size($fontsize, $maxsize, 0.77);
                break;
            case 'small':
                $size *= $this->multiply_font_size($fontsize, $maxsize, 0.86);
                break;
            case 'large':
                $size *= $this->multiply_font_size($fontsize, $maxsize, 1.2);
                break;
            case 'x-large':
                $size *= $this->multiply_font_size($fontsize, $maxsize, 1.5);
                break;
            case 'xx-large':
                $size *= $this->multiply_font_size($fontsize, $maxsize, 2);
                break;
            case 'px':
            default:
                $size *= 25.4 / $this->dpi;
    ...
    private function multiply_font_size($fontsize, $maxsize, $ratio)
    {
        if ($fontsize) {
